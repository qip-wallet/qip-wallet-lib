...
export const getInscriptionCost = ({fileSizes, feerate, padding, options}) => {
    try{
        ...
        if(options && options.service_fee){
            ...
            const prefix = getTransactionSize({input:1, output:[{outputType: "P2TR", count: 1}], addressType: "taptoot"}).txVBytes * feerate;
            total_fees += (options.service_fee * fileCount) + prefix
            outputCount += 1
        }
        ...
        if(options && options.collection_fee){
            ...
            const prefix = getTransactionSize({input:1, output:[{outputType: "P2TR", count: 1}], addressType: "taptoot"}).txVBytes * feerate;
            total_fees += (options.collection_fee * fileCount) + prefix
            outputCount += 1
        }
...
export const getInscriptions = async ({filePaths, publicKey, networkName, feerate, padding, options}) => {
    try{ 
        ...
        if(options && options.batch){
            ...
            count = options.batch.data.length
        ...
        if(options && options.service_fee){
            if(!options.service_address) throw new Error ("service_address is required to add a service fee")
            const prefix = getTransactionSize({input:1, output:[{outputType: "P2TR", count: 1}], addressType: "taptoot"}).txVBytes * feerate;
            total_fees += (options.service_fee * count) + prefix
            outputCount += 1
        }
        if(options && options.collection_fee){
            if(!options.collection_address) throw new Error ("collection_address is required to add a collection fee")
            const prefix = getTransactionSize({input:1, output:[{outputType: "P2TR", count: 1}], addressType: "taptoot"}).txVBytes * feerate;
            total_fees += (options.collection_fee * count) + prefix
            outputCount += 1
        }
    ...
    try{
        ...
        satUtxo.forEach(x => {
            if(x.txid === n_satpoint[0] && x.vout === parseInt(n_satpoint[1])){
                ...
                throw new Error ("satpoint not found")
            }
        })
...
const getFeeOut = async ({}) => {
    try{
        ...
        if(options && options.batch){
            if(options && options.service_fee){
                outputs.push({
                    value: options.service_fee * options.batch.data.length,
                    address: options.service_address
                });
            }
            if(options && options.collection_fee){
                outputs.push({
                    value: options.collection_fee * options.batch.data.length,
...
export const splitFunds = async ({filePaths, privateKey, networkName, feerate, padding, options}) => {
    try{
        ...
        let count 
        if(options && options.batch) {count = options.batch.data.length} else {count = inscriptions.length}

        if(options && options.service_fee){
            outputs.push({
                value: options.service_fee * count,
                address: options.service_address
            });
        }
        if(options && options.collection_fee){
            outputs.push({
                value: options.collection_fee * count,
                address: options.collection_address
            });
        }
...
const createBatchInscribeTx = async ({inscription, receiveAddress, privateKey, networkName}) => {
    try{
...
const createInscribeTx = async ({inscriptions, receiveAddress, privateKey, networkName}) => {
    try{
        ...
        for (let i = 0; i < inscriptions.length; i++) {
            ...
            console.log(inscription.inscriptionAddress)
            if(utxos.length === 0){
                throw new Error("No funds available for inscription")
            }
...
export const createInscribeTransactions = async ({filePaths, privateKey, receiveAddress, networkName, feerate, padding, options}) => {
    try{
        ...
        if(options && options.batch){
            ...
            transactions = await createBatchInscribeTx({inscription: inscription, receiveAddress: receiveAddress, privateKey: privateKey, networkName: networkName})
        }else{
...
// let feeRate = 5
// let padding = 550
// let publicKey = "464f03009f0693ffdcbfb5a2951295c1805741f1bae5015684b01d7ea19ff236"
// let privateKey = "4d03e27b296ee3b3f2bdf97c816e25a67b0c848b4632d4888c77c1cad009090f"
// let insc_fundAddr = "tb1pu28mpsacg02xwzeepwdwryullhvv67pujrlcqdd6ntgfshm8hnkq27ruru"
